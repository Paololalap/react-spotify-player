      ...
      {songs.map((song) => (
        ...
        >
          <div className="hidden sm:flex sm:w-[15rem] sm:items-center sm:justify-start sm:space-x-4 ">
            <div className="group/play hidden sm:relative sm:block sm:w-4">
              <span className="hidden sm:mr-2 sm:block sm:text-gray-400 sm:group-hover:text-[0.1px]">
                {song.id}
              </span>
              {/* Play button */}
              ...
              {/* Song title on hover */}
              <span className="hidden sm:invisible sm:absolute sm:left-[-35px] sm:top-[-15px] sm:z-10 sm:inline sm:w-auto sm:-translate-y-full sm:whitespace-nowrap  sm:rounded-[0.2rem] sm:bg-[#1f1f1f] sm:px-[0.4rem] sm:py-[0.1rem] sm:text-center sm:text-[15px] sm:font-[500] sm:opacity-0 sm:transition-opacity sm:delay-300 sm:group-hover/play:visible sm:group-hover/play:opacity-100">
            ...
            <div className="group/nest hidden sm:relative sm:block">
              <span className="hidden sm:mt-0 sm:block sm:cursor-pointer sm:whitespace-nowrap sm:text-[1rem] sm:text-white sm:hover:underline sm:hover:decoration-2">
                {song.title}
              </span>
              {/* Song title on hover */}
              <span className="sm:text-md hidden sm:invisible sm:absolute sm:left-1/2 sm:top-[-5px] sm:z-20 sm:inline sm:w-auto sm:-translate-x-1/2 sm:-translate-y-full sm:whitespace-nowrap sm:rounded-[0.2rem] sm:bg-[#1f1f1f] sm:px-[0.4rem] sm:py-[0.1rem] sm:text-center sm:font-[500] sm:opacity-0 sm:transition-opacity sm:delay-1000 sm:group-hover/nest:visible sm:group-hover/nest:opacity-100 ">
                {song.title}
              </span>
            </div>
          </div>
          {/* Song plays */}
          <span className="hidden sm:mt-0 sm:block sm:font-bold sm:text-gray-400 sm:group-hover:text-white">
            {song.plays}
          </span>
          {/* Action buttons */}
